...
Packages are a bit more minimal this time. `dplyr`, `readr`, and `lay` to work with data and `ggplot2` and `shadowtext` to create plots and `scales` and `gt` when building tables. This time we'll also include `spaMM`, which we'll use for some basic statistical modelling. Again, `countrycode` is needed to convert NOC to ISO3c.
...
historical_data_score <- historical_data |>
  filter(Year >= 1996) |> 
  mutate(score_prop = lay::lay(pick(c(perc_Gold, perc_Silver, perc_Bronze)), \(x) sum(x * c(9/13, 3/13, 1/13))))
...
print(paste("Correlation Olympic score and GDP:", round(cor(historical_data_gdp$score_prop, historical_data_gdp$gdp_bil), 2)))
```

Unsurprisingly, our single Olympic Score&trade; is strongly correlated with GDP (`r round(cor(historical_data_gdp$score_prop, historical_data_gdp$gdp_bil), 2)`), with countries that have higher GDP tending to have a higher Olympic Score&trade;.
...
```{r}
model <- spaMM::fitme(score_prop ~ gdp_bil + (1|Year), data = historical_data_gdp)
```
...
paris_predicted <- paris_predicted |> 
  ...
  mutate(score_diff_abs = score_prop - exp_score,
         score_diff_rel = score_prop/exp_score)

paris_predicted |> 
  select(country, gdp_bil, score_prop, exp_score)
```
...
pred_data$score_prop <- as.numeric(predict(model, newdata = pred_data, re.form = NA))
...
ggplot() +
  geomtextpath::geom_textpath(data = pred_data |> 
                                filter(gdp_bil > 6500),
                              aes(x = gdp_bil, y = score_prop, label = "Predicted Olympic score"),
                              text_only = TRUE, hjust = 0, vjust = 0, size = 3) +
  geom_line(data = pred_data,
            aes(x = gdp_bil, y = score_prop)) +
  geom_ribbon(data = pred_data,
              aes(x = gdp_bil, ymin = score_prop, ymax = Inf),
              fill = "darkgreen", alpha = 0.25) +
  geom_ribbon(data = pred_data |> bind_rows(data.frame(gdp_bil = Inf,
                                                       score_prop = max(pred_data$score_prop))),
              aes(x = gdp_bil, ymin = score_prop, ymax = -Inf),
  ...
  geom_point(data = paris_perc_gdp,
             aes(x = gdp_bil, y = score_prop),
             shape = 21, size = 1, stroke = 0.75) +
  ggimage::geom_image(data = paris_perc_gdp_png,
                      aes(x = gdp_bil, y = score_prop, image = imgURL), size = 0.05) +
  geom_text(data = paris_perc_gdp_png,
            aes(x = gdp_bil*1.5, y = score_prop, label = country),
            fontface = "bold", family = "Chivo", hjust = 0) +
  ...
  theme(plot.title = element_text(size = 20, family = "Chivo", face = "bold"),
        ...
        axis.title = element_text(size = 10, face = "bold", family = "Chivo"),
        axis.text = element_text(colour = "black", size = 8, family = "Chivo"))
```

In the above plot, the solid black line shows the predicted Olympic score from our model based on country GDP. Each point shows the *actual* Olympic score of a country at Paris 2024. Points above the line (green area) show countries that scored higher than expected, given their GDP. Point below the line (red area) are countries that had lower scores than our model predicted.
...
best_and_worst <- paris_predicted |> 
  ...
  slice(1:5, (n()-4):n()) |> 
  select(country:perc_Total, score_prop, exp_score, score_diff_rel, score_diff_abs)
...
paris_perc_gdp_png <- paris_perc_gdp |> 
  ...
  mutate(x = case_when(ISO3c == "QAT" ~ gdp_bil*0.675,
                       TRUE ~ gdp_bil * 1.1),
         y = case_when(ISO3c == "QAT" ~ score_prop - 0.2,
                       TRUE ~ score_prop + 0.2))
...
ggplot() +
  ...
  geom_point(data = paris_perc_gdp,
             aes(x = gdp_bil, y = score_prop),
             shape = 21, size = 1, stroke = 0.75, alpha = 0.25) +
  ggimage::geom_image(data = paris_perc_gdp_png,
                      aes(x = gdp_bil, y = score_prop, image = imgURL), size = 0.075) +
  geom_text(data = paris_perc_gdp_png,
            aes(x = x, y = y, label = country),
            fontface = "bold", family = "Chivo", hjust = 0, size = 3) +
  labs(title = "Paris 2024: Expectation v. Reality",
       subtitle = "Uzbekistan scored 5x higher than expected, while India\nonly matched 5% of expectations.",
       y = "Olympic score (weighted mean of medal %)", x = "GDP (Billion $USD)") +
  scale_x_log10() +
...
We can now see those countries that most exceeded or fell short of their Olympic expectations. The outcome of the New Zealand campaign is a direct comparison to that of Qatar and Peru. In 2023, all three countries had a GDP around 250 billion $USD, yet New Zealand went home with 10 Gold medals and Qatar and Peru failed to win a single Gold or Silver medal.
...
plot_data <- best_and_worst |> 
  ...
  select(country, ISO3c, score_prop, exp_score, score_diff_rel) |> 
  left_join(flag_db, by = c("ISO3c" = "Code")) |> 
  select(flag_URL, country, score_prop:score_diff_rel) |> 
...
plot_data |> 
  ...
  fmt_number(columns = c(score_prop, exp_score),
             decimals = 2) %>%
  ...
  cols_width(c(country) ~ px(125),
             c(score_prop, exp_score, score_diff_rel) ~ px(100)) %>% 
  tab_options(
...
A simple model with national GDP is a fairly crude method to estimate Olympic performance. It doesn't account for a countries level of development and how much of GDP is invested in sports. In the case of India, it also fails to consider whether the sports invested in (i.e. cricket) are included in the Olympics. However, it goes begin to account for a countries resources and funding that could go towards training Olympic athletes, and does seem to provide some (basic) predictive power. Adding more sophisticated data would be ideal, but finding this type of data for close to 100 countries is easier said than done. Mybe we can reconvene again in 4 years time to refine and improve our Olympic modelling.
...
How does our weighted Olympics Score&trade; compare traditional medal tally ranks?
...
score_diffs <- historical_data_score |> 
  ...
  ungroup() |> 
  arrange(desc(Year), desc(score_prop)) |> 
  group_by(Year) |> 
...
plot_data |> 
  ...
  fmt_number(columns = c(perc_Gold, perc_Silver, perc_Bronze, score_prop),
             decimals = 2) %>%
  opt_all_caps() %>% 
  opt_table_font(
    font = list(
